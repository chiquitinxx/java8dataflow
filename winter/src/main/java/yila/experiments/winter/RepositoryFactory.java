package yila.experiments.winter;

import java.util.Collections;

/**
 * JFL 17/11/18
 */
public class RepositoryFactory {

    private static PersonRepository personRepository;

    public static Result<?> onPersonRepository(PersonRepositoryFunction<PersonRepository, Result<?>> consumer) {

        try {
            return consumer.apply(personRepository);
        } catch (RepositoryException repositoryException) {
            //System.out.println(repositoryException.getMessage());
        }
        return Result.errors(Collections.singletonList(Error.ERROR_PERSON_REPOSITORY));
    }

    public static void setPersonRepository(PersonRepository repository) {
        personRepository = repository;
    }
}
